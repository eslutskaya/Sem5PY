...
for char in txt:
  if symbols.count(char) == 0:
    ...
    counts[symbols.index(char)] += 1
print(symbols, counts)
###txt = '''kAewtloYgcFQaJNhHVGxXDiQmzjfcpYbzxlWrVcqsmUbCunkfxZWDZjUZMiGqhRRiUvGmYmvnJIHEmbT
...
res_str = ''
for i in range(len(txt)):
  if ((not txt[i-4].istitle()) and txt[i-3].istitle() and txt[i-2].istitle() and txt[i-1].istitle()) and (not txt[i].istitle()) and (txt[i+1].istitle() and txt[i+2].istitle() and txt[i+3].istitle() and (not txt[i+4].istitle())) and txt[i].isalpha():
    res.append(txt[i])
    print(txt[i-3].istitle(),txt[i-2].istitle(),txt[i-1].istitle(),not txt[i].istitle(),txt[i+1].istitle(),txt[i+2].istitle(),txt[i+3].istitle())
print(res_str.join(res))
